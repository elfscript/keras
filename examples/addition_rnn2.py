...
print(X_val.shape) #, (5k, 7, 12)
print(y_val.shape) #, (5k, 4, 12)
print(X_val[0,:,:]) # (7,12)
...
for iteration in range(1, 20):
    ...
    #layer_output = get_layer_output([X_val[0:BATCH_SIZE]])
    ...
    for i in range(10):
        ...
        print('random index from [0, len(X_val) )= ', ind)
        #ind = np.random.randint(0, BATCH_SIZE)
        rowX, rowy = X_val[np.array([ind])], y_val[np.array([ind])]
        layer_output = get_layer_output([rowX])
        ...
        g2= ctable.decode(layer_output[0][0,:,:]) # (1,4,12) --> (4,12)
        print("g2",g2)
